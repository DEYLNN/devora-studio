...
function EditModelCard({ model, providers }) {
    ...
    return <div className="rounded-2xl border border-white/10 bg-white/[.03] p-4">
        <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 gap-3"><ModelIcon category={form.category} /><div className="min-w-0"><div className="truncate font-bold text-white">{form.display_name}</div><div className="mt-1 text-xs text-slate-500">{model.provider?.name} · {form.model_id}{model.version_rank ? ` · v${model.version_rank}` : ''}</div><div className="mt-2 flex flex-wrap gap-1 text-[10px] font-black uppercase tracking-[.12em]"><span className="rounded-full bg-white/10 px-2 py-1 text-slate-300">{categoryMeta(form.category).label}</span>{form.supports_vision && <span className="rounded-full bg-cyan-300/10 px-2 py-1 text-cyan-100">image</span>}{form.supports_files && <span className="rounded-full bg-violet-300/10 px-2 py-1 text-violet-100">file</span>}{model.is_latest && form.is_active && <span className="rounded-full border border-orange-300/20 bg-orange-300/10 px-2 py-1 text-orange-100">NEW</span>}{form.is_default && <span className="rounded-full bg-emerald-300/10 px-2 py-1 text-emerald-100">default</span>}{!form.is_active && <span className="rounded-full bg-red-300/10 px-2 py-1 text-red-100">inactive</span>}</div></div></div>
            <button onClick={() => setOpen((v) => !v)} className="rounded-full border border-white/10 px-3 py-1 text-xs font-bold text-slate-300 hover:bg-white/10">{open ? 'Close' : 'Edit'}</button>
        </div>
        {open && <div className="mt-5 grid gap-4 border-t border-white/10 pt-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6"><Toggle checked={form.supports_vision} onChange={(v)=>set('supports_vision', v)} label="Image" hint="show + upload" /><Toggle checked={form.supports_files} onChange={(v)=>set('supports_files', v)} label="Files" hint="future file upload" /><Toggle checked={form.supports_streaming} onChange={(v)=>set('supports_streaming', v)} label="Streaming" hint="SSE capable" /><Toggle checked={form.is_default} onChange={(v)=>set('is_default', v)} label="Default" hint="global chat default" /><Toggle checked={form.is_latest} onChange={(v)=>set('is_latest', v)} label="Latest" hint="one per category" /><Toggle checked={form.is_active} onChange={(v)=>set('is_active', v)} label="Active" hint="visible in chat" /></div>
            <button onClick={save} className="rounded-2xl bg-cyan-300 px-4 py-3 font-black text-slate-950">Save changes</button>
        </div>}
    </div>;
}

export default function Index({ models = [], providers = [] }) {
    const { data, setData, post, processing, errors, reset } = useForm({ provider_id: providers[0]?.id ?? '', model_id: '', display_name: '', category: 'generic', context_window: '', supports_vision: false, supports_files: false, supports_streaming: true, is_default: false, is_latest: false, is_active: true });
    const submit = (e) => { e.preventDefault(); post(route('admin.models.store'), { onSuccess: () => reset('model_id','display_name','context_window') }); };
    return (
        <AuthenticatedLayout header={<div><p className="text-xs font-bold uppercase tracking-[.35em] text-cyan-200/70">Admin</p><h2 className="mt-2 text-2xl font-black tracking-[-.04em] text-white">AI Models</h2></div>}>
            ...
            <div className="mx-auto grid max-w-7xl gap-4 p-4 lg:grid-cols-[420px_1fr]">
                ...
                <div className="rounded-[1.75rem] border border-white/10 bg-slate-950/70 p-5 shadow-2xl backdrop-blur-xl">
                    <div className="mb-4 flex items-center justify-between"><h3 className="font-bold text-white">Model Catalog</h3><Link href={route('admin.providers.index')} className="text-sm text-cyan-200">Providers →</Link></div>
